...
import { BookOpen, Star, CheckCircle, ChevronLeft } from "lucide-react";
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <div className="flex-1">
                    ...
                    {item.type === "review" && (
                      <div className="flex items-center gap-1 mt-1">
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <span key={s} className={`text-[10px] ${s <= (item.detail.rating ?? 0) ? "text-amber" : "text-border"}`}>★</span>
                          ))}
                        </div>
                        {item.detail.excerpt && (
                          <p className="text-xs text-ink-muted mt-1 line-clamp-2 leading-relaxed">"{item.detail.excerpt}"</p>
                        )}
                      </div>
                    )}
                    {item.type === "finish" && (
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      <div className="flex items-center gap-1 text-ink-muted/40">{label.icon}</div>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="space-y-3">
        ...
        {items.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-border p-6 text-center">
            <p className="text-sm text-ink-muted">Belum ada aktivitas. Ikuti pengguna lain untuk melihat aktivitas mereka.</p>
          </div>
        ) : (
