    ...
    if (book) {
      ...
      const promises: Promise<unknown>[] = [
        insertActivity(memberId, familyId, "log", {
          book_id: book.id, book_title: book.title, book_slug: slug, book_cover: book.cover_url, pages_read: pagesRead, duration_minutes: durationMinutes ?? null, from_page: fromPage ?? null, to_page: toPage ?? null, images: images ?? null, note: note ?? null, log_id: log.id,
        }, admin),
      ];
      if (autoFinished) {
