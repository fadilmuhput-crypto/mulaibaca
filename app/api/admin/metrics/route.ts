...
export async function GET() {
  ...
  const [
    ...
    supabase
      .from("reading_logs")
      .select("pages_read, duration_minutes, member_id")
      .eq("log_date", today),
    supabase
      .from("reading_logs")
      .select("log_date, pages_read, member_id")
      .gte("log_date", _30dAgo),
    supabase
      .from("reading_logs")
      .select("pages_read, duration_minutes"),
    supabase
      .from("streaks")
      .select("current_streak, longest_streak, member_id"),
    supabase
      .from("members")
      .select("id, auth_user_id, family_id, weekly_pages_goal"),
    supabase
      .from("books")
      .select("enrichment_status, id"),
    supabase
      .from("shelf_items")
      .select("status, member_id, book_id"),
    supabase
      .from("reviews")
      .select("rating, member_id, published_at, shelf_item_id"),
    supabase
      .from("reviews")
      .select("published_at")
      .gte("published_at", _30dAgo),
    supabase
      .from("notifications")
      .select("type, is_read"),
    supabase
      ...
      .gte("created_at", _30dAgo),
    supabase
      .from("members")
      .select("created_at, member_type")
  ...
  const avgPages = sLogs.length
    ? Math.round((sLogs.reduce((s, l) => s + l.pages_read, 0) / sLogs.length) * 10) / 10
    : 0;
  const avgDur = sLogs.length
    ? Math.round((sLogs.reduce((s, l) => s + (l.duration_minutes ?? 0), 0) / sLogs.length) * 10) / 10
    : 0;

  // ── Activity daily (last 30 days) ──
  ...
  const avgStreak = regStreaks.length
    ? Math.round((regStreaks.reduce((s: number, r: { current_streak: number }) => s + r.current_streak, 0) / regStreaks.length) * 10) / 10
    : 0;
  ...
  const avgRating = totalReviews
    ? Math.round(((allReviews ?? []).reduce((s: number, r: { rating: number }) => s + r.rating, 0) / totalReviews) * 10) / 10
    : 0;
  const ratingDist: Record<string, number> = {};
  for (const r of allReviews ?? []) {
    const k = String((r as { rating: number }).rating);
    ratingDist[k] = (ratingDist[k] ?? 0) + 1;
  }
  const reviewsPerDay: Record<string, number> = {};
  for (const r of reviews30d ?? []) {
    const d = (r as { published_at: string }).published_at.slice(0, 10);
    reviewsPerDay[d] = (reviewsPerDay[d] ?? 0) + 1;
  }
  ...
  for (const m of membersAll ?? []) {
    const mem = m as { family_id: string; id: string };
    familyMemberCounts[mem.family_id] = (familyMemberCounts[mem.family_id] ?? 0) + 1;
  }
  const punyaKeluarga = new Set(
    (membersAll ?? [])
      .filter((m: { family_id: string; id: string }) => familyMemberCounts[m.family_id] > 1)
  ...
  // ── Keluarga size distribution ──
  const famSizeCount: Record<string, { size: number; count: number }> = {};
  for (const m of membersAll ?? []) {
    const mem = m as { family_id: string };
    if (!famSizeCount[mem.family_id]) famSizeCount[mem.family_id] = { size: 0, count: 0 };
    famSizeCount[mem.family_id].size++;
  }
  Object.values(famSizeCount).forEach((f) => f.count++);
  const familySizeDist: Record<string, number> = {};
  for (const f of Object.values(famSizeCount)) {
    const key = f.size === 1 ? "1" : f.size === 2 ? "2" : f.size <= 4 ? "3-4" : f.size <= 6 ? "5-6" : "7+";
    familySizeDist[key] = (familySizeDist[key] ?? 0) + 1;
  }

  // ── Notifications ──
  const notifByType = (notifData ?? []).reduce(
    (acc: Record<string, { total: number; read: number }>, n: { type: string; is_read: boolean }) => {
  ...
  for (const f of families ?? []) {
    const d = (f as { created_at: string }).created_at.slice(0, 10);
    familiesPerDay[d] = (familiesPerDay[d] ?? 0) + 1;
  }
  const membersPerDay: Record<string, number> = {};
  for (const m of registeredMembers ?? []) {
    const d = (m as { created_at: string }).created_at.slice(0, 10);
    membersPerDay[d] = (membersPerDay[d] ?? 0) + 1;
  }
