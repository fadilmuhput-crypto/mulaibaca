...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { curated_book_id } = await req.json();

  if (!curated_book_id) {
    return NextResponse.json({ error: "curated_book_id wajib diisi" }, { status: 400 });
  }
  ...
  const { error } = await admin
    .from("jelajah_section_books")
    .insert({ section_id, curated_book_id, sort_order: count ?? 0 });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await admin
    ...
    .delete()
    .eq("section_id", section_id)
    .eq("curated_book_id", curated_book_id);
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id: section_id } = await params;
  const { order } = await req.json() as { order: { curated_book_id: string; sort_order: number }[] };

  const admin = createAdminClient();
  const updates = order.map(({ curated_book_id, sort_order }) =>
    admin
      .from("jelajah_section_books")
      .update({ sort_order })
      .eq("section_id", section_id)
