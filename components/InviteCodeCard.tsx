...
export default function InviteCodeCard({
  ...
  const shareText = `Yuk gabung ke ${familyName} di mulaibaca! 📚\n\nKlik link ini:\n${shareUrl}`;
  ...
  return (
    <section className="bg-forest rounded-2xl p-5 text-white">
      ...
      <div className="flex gap-2">
        <button
          onClick={handleShare}
          className="flex-1 bg-amber text-white font-medium rounded-xl min-h-[44px] text-sm hover:bg-amber-hover transition-colors"
        >
          Bagikan →
        </button>
        <a
          ...
          className="flex items-center justify-center gap-1.5 bg-[#25D366] text-white font-medium rounded-xl min-h-[44px] text-sm hover:bg-[#20bd5a] transition-colors px-4"
        >
      ...
      <p className="text-xs text-white/50 text-center mt-3">
        Mereka daftar di mulaibaca.id/bergabung?code={inviteCode.toLowerCase()}
      </p>
    </section>
  );
}
